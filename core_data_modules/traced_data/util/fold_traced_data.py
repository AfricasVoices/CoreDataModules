...
class FoldStrategies(object):
    ...
    def list_of_labels(scheme, x, y):
        ...
        if len(x) == 1 and scheme.get_code_with_code_id(x[0]["CodeID"]).control_code == Codes.TRUE_MISSING and \
            ...
            return [x]
        x = [l for l in x if scheme.get_code_with_code_id(l["CodeID"]).control_code != Codes.TRUE_MISSING]
        y = [l for l in y if scheme.get_code_with_code_id(l["CodeID"]).control_code != Codes.TRUE_MISSING]
