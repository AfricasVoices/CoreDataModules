...
class TestTracedDataCodaV2IO(unittest.TestCase):
    ...
    def test_export_two_single_coded_schemes(self):
        ...
        # Export to a Coda 2 messages file
        with open(file_path, "w") as f:
            scheme_map = collections.OrderedDict()  # Using OrderedDict to make tests easier to write in Py2 and Py3.
            scheme_map["district"] = district_scheme
            scheme_map["zone"] = zone_scheme

            TracedDataCodaV2IO.export_traced_data_iterable_to_coda_2(
                messages, "location_raw", "location_sent_on", "location_coda_id", scheme_map, f)
        ...
        def test_ambiguous_missing_codes(d):
            ...
            with open(file_path, "w") as f:
                try:
                    scheme_map = collections.OrderedDict()
                    scheme_map["district"] = district_scheme
                    scheme_map["zone"] = zone_scheme

                    TracedDataCodaV2IO.export_traced_data_iterable_to_coda_2(
                        conflicting_missings, "location_raw", "location_sent_on", "location_coda_id", scheme_map, f)
                except AssertionError as e:
